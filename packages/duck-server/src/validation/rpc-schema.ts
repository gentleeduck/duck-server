...
/**
 * Helpers for using the standard schema
 */
...
/** Parse input. */
export async function parseInput<S extends AnySchema>(schema: S, raw: unknown): Promise<InferOut<S>> {
  const res = await schema['~standard'].validate(raw)
...
/** Parse output. */
export async function parseOutput<S extends AnySchema>(schema: S, raw: unknown): Promise<InferOut<S>> {
  const res = await schema['~standard'].validate(raw)
  if (!res.issues) return res.value
